...
/**
 * Shared "passively fertilize the plant above me" behavior used by every
...
public final class GrowthBooster {
    ...
    /**
     * Attempts one bone-meal-equivalent growth stage on whatever is directly
     * above {@code pos}, with probability {@code chance} per call. Mirrors the
     * check-and-apply sequence vanilla's BoneMealItem uses on a right-click,
     * minus the item-consumption side effects, since no item is involved here.
     *
     * <p>The random roll happens before any world/block-state reads, so the
     * common case (a failed roll at low chance values) costs nothing.
     */
    public static void tryBoostAbove(final ServerLevel world, final BlockPos pos, final RandomSource random, final float chance) {
